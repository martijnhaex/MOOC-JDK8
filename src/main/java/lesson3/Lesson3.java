...
public class Lesson3 {
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        // YOUR CODE HERE
        streamOfWords(wordList, parallel)
                .map(w -> IntStream.range(0, LIST_SIZE).map(index -> Levenshtein.lev(w, wordList.get(index))).toArray())
    ...
    static List<String> processWords(List<String> wordList, boolean parallel) {
        // YOUR CODE HERE
        return streamOfWords(wordList, parallel).map(String::toLowerCase).distinct().collect(Collectors.toList());
    }

    private static Stream<String> streamOfWords(List<String> wordList, boolean parallel) {
